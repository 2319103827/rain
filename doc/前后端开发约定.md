...
$.getJSON('/ajax/news/', function (data) {
	$.each(data, function () {
    	...
    	this.url = APP_PATH + 'news/?id=' + this.id;
    })
})
```
...
例如 `task_status` 会有 `success` `wait` `fail` 三个状态。则只能新增 `delete`状态，不能修改 `success` 为 `done`。因为前端通过编写匹配规则，如下代码：
...
一旦 `success` 被修改为 `done`，前端必须修改 JavaScript 中的匹配规则代码，而有时前端匹配代码散落在很多地方，不一定能找到所有代码的 `success` 修改为 `done`。故此，状态信息（标识信息）只需增加不许修改。

例如不要将 `task_status` 与 管理后台可控制的 `task_status_msg` 状态文字共用。
...
后端接口只提供动态数据。静态数据全部由 View (模板直接编写)。
...
```javascript
// 已确定 用户中心的 URL 是固定不变的，所以直接卸载模板里面
<a href="/user/">用户中心</a>
```
